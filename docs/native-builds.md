...
The picker signs in to Expo and reads updates and native builds directly from EAS. Build matching requires the exact EAS project, runtime, iOS platform, and configured build profile. An unrelated newer build is never substituted. GitHub automatically requests a matching build after successful same-repository PR publication and also accepts manual requests. It does not host the discovery data.
...
2. PR publication starts the matching build lookup automatically. While it runs, wait for the build status to appear. If that automation failed or this draft was published manually, select **Request Build**. Safari opens a prefilled GitHub issue with the preview's identity. Sign in to GitHub and submit the issue. Merely opening the page does not create a build.
3. For automatic requests, GitHub validates the successful same-repository publication and its EAS output. For manual requests, it verifies that the requester has write, maintain, or admin access. It queries the latest iOS update on the requested EAS channel, verifies the exact update ID and runtime, and uses EAS's source commit. The channel must be active and route to one branch without a rollout or rollback. PRs must still point to that exact commit in the same repository. A stale request must be refreshed.
...
5. GitHub serializes requests for the same project, iOS platform, profile, and runtime without canceling the running build. EAS `get-build` looks for that exact runtime and internal device profile, reusing a completed build or waiting for a running one. A matching build can serve multiple PRs. If necessary, the `build` job creates an iPhone build using configured remote signing credentials.
...
7. Complete installation on the phone and reopen the app. Once the matching runtime and first screen are ready, the picker automatically opens the saved exact publication. The new native binary determines which other updates are compatible.
...
Before handing the installer URL to iOS, the picker saves the project, source and target runtimes, build ID, channel, and exact iOS update UUID. The **Installation requested** indicator is limited to 15 minutes; iOS provides no confirmation or download-progress callback. The saved auto-open selection remains available for seven days and can be canceled with **Cancel Auto-Open**. Canceling that selection does not cancel an installation already accepted by iOS.

On the target runtime, the app waits for React content and Expo startup to finish, verifies project access and the exact publication through EAS, then uses the existing transactional update switch. It clears the selection only after observing the exact launched UUID and runtime. If the PR was republished meanwhile, it asks the user to choose again instead of opening a different update. Offline, authentication, and download failures retain an explicit retry action. An interrupted attempt never repeats automatically on each launch. Choosing another draft, running the bundled version, signing out, or canceling clears the saved selection; an expired session retains it for sign-in and retry.

The app retains its current bundle while waiting or after a failed attempt. Installing an unrelated native runtime discards the old selection. Older installation records without an exact update UUID remain status-only and cannot trigger auto-open. The source build that initiates installation must include this feature; installing the feature for the first time cannot recover an update ID that the previous build did not save.
...
Enable GitHub Issues and add the repository secret `EXPO_TOKEN` with project build access. The request workflow grants `contents: read`, `issues: read`, and `actions: read`. Automatic requests verify the successful same-repository publication and its EAS output; manual requests verify the actor's repository permission. Public users can open issues but cannot start builds without repository write access.
...
Automatic PR builds set `refresh_ad_hoc_provisioning_profile: false` to reuse the existing valid profile. Manual runs and issue-triggered requests default to `true`, which requires an App Store Connect API key assigned to the app for EAS to refresh its managed profile without an interactive Apple sign-in. See [Expo's internal distribution and CI requirements](https://docs.expo.dev/build/internal-distribution/).
...
The app hands a verified EAS installation manifest to iOS using `itms-services`. iOS downloads and installs the signed app after confirmation. Reopen the app after installation to resume the selected draft. TestFlight is not required for this ad hoc workflow.
