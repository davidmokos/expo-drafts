...
swiftc -emit-module -emit-library -module-name EXUpdates tests/ios/EXUpdatesTestDouble.swift \
  ...
  "$policy_dir/LauncherSelectionPolicyFilterAware.swift" "$policy_dir/LoaderSelectionPolicy.swift" \
  "$policy_dir/LoaderSelectionPolicyFilterAware.swift" \
  -o "$test_dir/libEXUpdates.dylib" -emit-module-path "$test_dir/EXUpdates.swiftmodule"
